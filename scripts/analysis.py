...
from constants import NUM_SIMULATIONS, DAYS_AHEAD, TIME_INCREMENT, DEBTS, LIQUIDITIES, COLLATERALIZATION_RATIO, QUANTITY_RESERVE_ASSET, OC_LEVELS, TOKEN_BASKET
...
worst_eth_shock = get_data.create_logrets_df()[START_DATE_DATA:END_DATE_DATA]['ETH'].sort_values()[0]

#4. Using dataframe for crash period, perform Monte Carlo simulation
price_simulations = simulation.multivariate_monte_carlo(historical_prices = crash_df, num_simulations = NUM_SIMULATIONS, T = DAYS_AHEAD, dt = TIME_INCREMENT, length_of_crash = 1, shock_size = worst_eth_shock)
